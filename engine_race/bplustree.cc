...
#define key(node) ((my_key_t *)offset_ptr(node))
#define data(node) ((size_t*)(offset_ptr(node) + _max_entries * sizeof(my_key_t)))
#define sub(node) ((off_t *)(offset_ptr(node) + (_max_order - 1) * sizeof(my_key_t)))
...
static int key_binary_search(struct bplus_node *node, my_key_t target)
{
        my_key_t *arr = key(node);
...
static inline int parent_key_index(struct bplus_node *parent, my_key_t key)
{
...
static size_t bplus_tree_search(struct bplus_tree *tree, my_key_t key, bool &flag)
{
        size_t ret = -1;
...
static my_key_t non_leaf_insert(struct bplus_tree *tree, struct bplus_node *node,
...
static my_key_t non_leaf_split_left(struct bplus_tree *tree, struct bplus_node *node,
                	         struct bplus_node *left, struct bplus_node *l_ch,
                	         struct bplus_node *r_ch, my_key_t key, int insert)
{
        int i;
        my_key_t split_key;
        ...
        memmove(&key(left)[0], &key(node)[0], pivot * sizeof(my_key_t));
        memmove(&sub(left)[0], &sub(node)[0], pivot * sizeof(off_t));

        /* replicate from key[insert] to key[split - 1] in original node */
        memmove(&key(left)[pivot + 1], &key(node)[pivot], (split - pivot - 1) * sizeof(my_key_t));
        ...
        memmove(&key(node)[0], &key(node)[split - 1], (node->children - 1) * sizeof(my_key_t));
        memmove(&sub(node)[1], &sub(node)[split], (node->children - 1) * sizeof(off_t));

        return split_key;
}

static my_key_t non_leaf_split_right1(struct bplus_tree *tree, struct bplus_node *node,
                        	   ...
                        	   struct bplus_node *r_ch, my_key_t key, int insert)
{
        ...
        my_key_t split_key = key(node)[split - 1];
        ...
        memmove(&key(right)[pivot + 1], &key(node)[split], (right->children - 2) * sizeof(my_key_t));
...
static my_key_t non_leaf_split_right2(struct bplus_tree *tree, struct bplus_node *node,
                        	   struct bplus_node *right, struct bplus_node *l_ch,
                        	   struct bplus_node *r_ch, my_key_t key, int insert)
{
        int i;
        ...
        my_key_t split_key = key(node)[split];
        ...
        memmove(&key(right)[0], &key(node)[split + 1], pivot * sizeof(my_key_t));
        ...
        memmove(&key(right)[pivot + 1], &key(node)[insert], (_max_order - insert - 1) * sizeof(my_key_t));
...
static void non_leaf_simple_insert(struct bplus_tree *tree, struct bplus_node *node,
                        	   ...
                        	   my_key_t key, int insert)
{
        memmove(&key(node)[insert + 1], &key(node)[insert], (node->children - 1 - insert) * sizeof(my_key_t));
...
static my_key_t leaf_split_left(struct bplus_tree *tree, struct bplus_node *leaf,
                	     struct bplus_node *left, my_key_t key, size_t data, int insert)
{
        /* split = [m/2] */
        ...
        memmove(&key(left)[0], &key(leaf)[0], pivot * sizeof(my_key_t));
        memmove(&data(left)[0], &data(leaf)[0], pivot * sizeof(size_t));
        ...
        memmove(&key(left)[pivot + 1], &key(leaf)[pivot], (split - pivot - 1) * sizeof(my_key_t));
        memmove(&data(left)[pivot + 1], &data(leaf)[pivot], (split - pivot - 1) * sizeof(size_t));

        /* original leaf left shift */
        memmove(&key(leaf)[0], &key(leaf)[split - 1], leaf->children * sizeof(my_key_t));
        memmove(&data(leaf)[0], &data(leaf)[split - 1], leaf->children * sizeof(size_t));

        //if(data==7971461820416)
        //    printf("data:%ld\n",data);

        return key(leaf)[0];
}

static my_key_t leaf_split_right(struct bplus_tree *tree, struct bplus_node *leaf,
                	      struct bplus_node *right, my_key_t key, size_t data, int insert)
{
        /* split = [m/2] */
        int split = (leaf->children + 1) / 2;
        ...
        memmove(&key(right)[0], &key(leaf)[split], pivot * sizeof(my_key_t));
        memmove(&data(right)[0], &data(leaf)[split], pivot * sizeof(size_t));
        ...
        memmove(&key(right)[pivot + 1], &key(leaf)[insert], (_max_entries - insert) * sizeof(my_key_t));
        memmove(&data(right)[pivot + 1], &data(leaf)[insert], (_max_entries - insert) * sizeof(size_t));
...
static void leaf_simple_insert(struct bplus_tree *tree, struct bplus_node *leaf,
                	       my_key_t key, size_t data, int insert)
{
        //if(data==7971461820416)
        //    printf("s-insert data:%ld\n",data);
        memmove(&key(leaf)[insert + 1], &key(leaf)[insert], (leaf->children - insert) * sizeof(my_key_t));
        memmove(&data(leaf)[insert + 1], &data(leaf)[insert], (leaf->children - insert) * sizeof(size_t));
        key(leaf)[insert] = key;
        data(leaf)[insert] = data;
        leaf->children++;
}

static int leaf_insert(struct bplus_tree *tree, struct bplus_node *leaf, my_key_t key, size_t data)
{
        /* Search key location */
...
static int bplus_tree_insert(struct bplus_tree *tree, my_key_t key, size_t data)
{
        struct bplus_node *node = node_seek(tree, tree->root);
        while (node != NULL) {
                if (is_leaf(node)) {
                        	     ...
                        	     int parent_key_index, int remove)
{
        ...
        memmove(&key(node)[1], &key(node)[0], remove * sizeof(my_key_t));
                        	     ...
                        	     int parent_key_index, int remove)
{
        ...
        memmove(&key(left)[left->children], &key(node)[0], remove * sizeof(my_key_t));
        memmove(&sub(left)[left->children], &sub(node)[0], (remove + 1) * sizeof(off_t));

        /* sub-node sum = node->children - 1 */
        memmove(&key(left)[left->children + remove], &key(node)[remove + 1], (node->children - remove - 2) * sizeof(my_key_t));
                        	      ...
                        	      int parent_key_index)
{
        ...
        memmove(&key(right)[0], &key(right)[1], (right->children - 2) * sizeof(my_key_t));
                        	      ...
                        	      int parent_key_index)
{
        ...
        memmove(&key(node)[node->children - 1], &key(right)[0], (right->children - 1) * sizeof(my_key_t));
...
static inline void non_leaf_simple_remove(struct bplus_tree *tree, struct bplus_node *node, int remove)
{
        ...
        memmove(&key(node)[remove], &key(node)[remove + 1], (node->children - remove - 2) * sizeof(my_key_t));
                		 ...
                		 int parent_key_index, int remove)
{
        ...
        memmove(&key(leaf)[1], &key(leaf)[0], remove * sizeof(my_key_t));
                		 ...
                		 struct bplus_node *left, int parent_key_index, int remove)
{
        ...
        memmove(&key(left)[left->children], &key(leaf)[0], remove * sizeof(my_key_t));
        memmove(&data(left)[left->children], &data(leaf)[0], remove * sizeof(off_t));
        memmove(&key(left)[left->children + remove], &key(leaf)[remove + 1], (leaf->children - remove - 1) * sizeof(my_key_t));
                                  ...
                                  int parent_key_index)
{
        ...
        memmove(&key(right)[0], &key(right)[1], (right->children - 1) * sizeof(my_key_t));
...
{
        memmove(&key(leaf)[leaf->children], &key(right)[0], right->children * sizeof(my_key_t));
...
{
        memmove(&key(leaf)[remove], &key(leaf)[remove + 1], (leaf->children - remove - 1) * sizeof(my_key_t));
        memmove(&data(leaf)[remove], &data(leaf)[remove + 1], (leaf->children - remove - 1) * sizeof(off_t));
        leaf->children--;
}

static int leaf_remove(struct bplus_tree *tree, struct bplus_node *leaf, my_key_t key)
{
...
static int bplus_tree_delete(struct bplus_tree *tree, my_key_t key)
{
        struct bplus_node *node = node_seek(tree, tree->root);
        while (node != NULL) {
                if (is_leaf(node)) {
...
{
        return bplus_tree_search(tree, key, flag);
}
        ...
        while (node != NULL) {
                ...
                if (is_leaf(node)) {
                        ...
                        while (node != NULL && key(node)[i] <= max) {
                                item.key = key(node)[i];
                                item.page = data(node)[i]>>32;
                                item.pos = data(node)[i]&(((size_t)1<<32)-1);
...
struct bplus_tree *bplus_tree_init(char *filename, int block_size)
{
        ...
        _max_order = (block_size - sizeof(node)) / (sizeof(my_key_t) + sizeof(off_t));
        _max_entries = (block_size - sizeof(node)) / (sizeof(my_key_t) + sizeof(size_t));
        if (_max_order <= 2) {
                fprintf(stderr, "block size is too small for one node!\n");
                return NULL;
        }
        ...
        if (fd >= 0) {
                ...
                /* load free blocks */
                while ((unsigned int)(i = offset_load(fd)) != INVALID_OFFSET) {
        ...
        _max_order = (_block_size - sizeof(node)) / (sizeof(my_key_t) + sizeof(off_t));
        _max_entries = (_block_size - sizeof(node)) / (sizeof(my_key_t) + sizeof(size_t));
